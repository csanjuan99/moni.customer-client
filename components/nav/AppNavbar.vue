...
const mutateIsShowSale = () => {
  isShowSale.value = !isShowSale.value;
}
...
<template>
  <nav :class="[ y > 0 ? '': 'bg-white']"
       class="fixed top-0 left-0 w-full h-24 px-3 py-3 transition-all duration-200 ease-in-out z-50">
    <div class="flex items-center justify-between w-full h-full rounded-xl bg-white px-7 z-50">
        ...
        <article class="flex gap-10">
          ...
          <div class="flex items-center">
            ...
            <div class="relative w-full">
                ...
                <input type="text" class="bg-white border-none text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-b focus:border-gray-300 block w-full pl-10 p-2.5" readonly placeholder="Search" @click=" () => { isShowFilter = !isShowFilter }" />
            </div>
          </div>
          
          <div class="flex gap-3">
            <svg fill="none" class="w-8" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"></path>
            </svg> 
            <svg fill="none" class="w-8" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" @click=" () => { isShowshopping = !isShowshopping }">
      ...
      <ModalsAppSaleModal :isShow="isShowSale" @mutateIsShowshopping="mutateIsShowSale"/>
