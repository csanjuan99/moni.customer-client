...
const mutateHoverElement = (status:boolean) => {
  hoverELement.value = status
}
...
<template>
  ...
  >
    ...
    <NuxtLink :to="`/products/${product.attributes.slug}`" class="h-auto overflow-hidden rounded-3xl z-0 relative">
      <picture>
        <img :class="hoverELement ? 'scale-125':''" class="h-full w-full object-cover transition-all  ease-in-out duration-300 rounded-3xl"
        :src="`${config.public.baseURL}${product.attributes.media.data[0].attributes.url}`"
        alt="" @mouseover="mutateHoverElement(true)" @mouseleave="mutateHoverElement(false)">
      </picture>
      <div :class="hoverELement ? 'visible':'invisible'" class="absolute bottom-0 left-0 w-full px-2 md:px-6 2xl:px-10 py-10 ease-in-out duration-100" @mouseover="mutateHoverElement(true)" @mouseleave="mutateHoverElement(false)">
        <div class="bg-white rounded-lg shadow-lg p-4">
          <h2>Agregar a la bolsa</h2>
          <div class="flex rounded-full mt-4 justify-center">
            <div v-for="(element,index) in sizes" :key="index" class="flex items-center justify-center w-10 h-10 text-black hover:text-white hover:bg-gray-400 rounded-full mx-2">
    ...
    <article class="flex flex-row justify-between p-2">
      <div class="flex flex-col items-start gap-2 h-max" @mouseover="mutateHoverElement(true)" @mouseleave="mutateHoverElement(false)">
        <h3 class="font-poppins text-xl text-gray-900">
          {{ product.attributes.name }}
        </h3>
        <div class="flex items-center gap-2 md:gap-5 text-xs sm:text-sm">
      ...
      <div class="flex gap-3">
        <svg fill="none" width="40" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
